...
def sprayChart(dates, firstname, lastname):
    data = process_data(dates, firstname, lastname, 'batter')
    if data.size == 0:
...
def drawChart(data, firstname, lastname):
    ...
    infieldDir = [Path.Path.MOVETO, Path.Path.CURVE3, Path.Path.CURVE3]
    infieldPath = Path.Path(infieldPts, infieldDir)
    ...
    outfieldDir = [Path.Path.MOVETO, Path.Path.CURVE3, Path.Path.CURVE3]
    outfieldPath = Path.Path(outfieldPts, outfieldDir)
...
sprayChart('2016-05-01 to 2017-05-01', 'dustin', 'pedroia')
